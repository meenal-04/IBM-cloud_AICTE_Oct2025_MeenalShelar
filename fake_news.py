...
# Import Groq client directly (OpenAI is no longer needed)
from groq import Groq
import time  # Added for potential use in spinner

# Loading environment variables
load_dotenv()

# --- START: CORRECTED SECRETS VALIDATION ---
try:
    GROQ_API_KEY = st.secrets.GROQ_API_KEY
    GOOGLE_API_KEY = st.secrets.GOOGLE_API_KEY
    CSE_ID = st.secrets.CSE_ID
except AttributeError:
    st.error("⚠️ Streamlit Secrets are missing. Please ensure GROQ_API_KEY, GOOGLE_API_KEY, and CSE_ID are set at the ROOT LEVEL in your Streamlit Cloud secrets configuration.")
    st.stop()
# --- END: CORRECTED SECRETS VALIDATION ---
...
try:
    ...
    st.error(f"FATAL CLIENT INITIALIZATION ERROR (Groq): {e}")
    st.stop()
